...
class Blockchain:
    ...
    def add_all_nodes(self):
        base_url = "127.0.0.1"
        base_port=5000
        ports = [a for a in range(base_port +1,base_port + 20 )]
        all_nodes = ['{0}:{1}'.format(base_url, a) for a in ports]
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Third', amount = 10)
